...
df['order_date'] = pd.to_datetime(df['order_date']) ## gunanya untuk viz timeseries
df['ship_date'] = pd.to_datetime(df['ship_date']) ## gunanya untuk viz timeseries

df['order_year'] = df['order_date'].dt.year ## extract year 2024-01-01 —> 2024
...
with mx_sales:
    ...
    prev_sales = data.loc[data['order_year']==PREV_YEAR, 'sales'].values[0] ## values[0] suapaya yg diambil bukan series atau dataframe tetapi valuenya
...
# syntax di bawah tidak dipakai, diganti dengan yang di bawahnya
